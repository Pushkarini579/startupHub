...
const getStartups = async (req, res) => {
    try {
        ...
        const startups = await Startup_1.Startup.find(query)
            .populate('founderId', 'name email profileImage')
            .sort({ createdAt: -1 })
            .skip(skip)
            .limit(limit);
        const total = await Startup_1.Startup.countDocuments(query);
...
const getStartupById = async (req, res) => {
    try {
        ...
        const startup = await Startup_1.Startup.findById(req.params.id).populate('founderId', 'name email profileImage');
        if (!startup) {
...
const updateStartup = async (req, res) => {
    try {
        ...
        const startup = await Startup_1.Startup.findById(req.params.id);
        if (!startup) {
            return res.status(404).json({ message: 'Startup not found' });
        }
        ...
        if (req.file) {
            startup.logo = await (0, cloudinary_1.uploadFile)(req.file.buffer, req.file.originalname, 'logos', req.file.mimetype);
        }
        await startup.save();
        return res.status(200).json({
...
const approveStartup = async (req, res) => {
    try {
        ...
        const startup = await Startup_1.Startup.findByIdAndUpdate(req.params.id, { status }, { new: true });
        if (!startup) {
            return res.status(404).json({ message: 'Startup not found' });
        }
        return res.status(200).json({
...
exports.approveStartup = approveStartup;
